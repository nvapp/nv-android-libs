...
import com.nvapp.android.libs.util.loc.KalmanLocationManager.UseProvider;
...
class LooperThread extends Thread {
	...
	/**
	 * Three 1-dimension trackers, since the dimensions are independent and can
	 ...
	 */
	LooperThread(Context context, UseProvider useProvider, long minTimeFilter, long minTimeGpsProvider,
			long minTimeNetProvider, LocationListener locationListener, boolean forwardProviderUpdates) {
		mContext = context;
	...
	public void run() {
		...
		if (mUseProvider == UseProvider.GPS || mUseProvider == UseProvider.GPS_AND_NET) {
			...
			mLocationManager.requestLocationUpdates(LocationManager.GPS_PROVIDER, mMinTimeGpsProvider, 0.0f,
					mOwnLocationListener, mLooper);
		}

		if (mUseProvider == UseProvider.NET || mUseProvider == UseProvider.GPS_AND_NET) {

			mLocationManager.requestLocationUpdates(LocationManager.NETWORK_PROVIDER, mMinTimeNetProvider, 0.0f,
					mOwnLocationListener, mLooper);
		}
	...
	private LocationListener mOwnLocationListener = new LocationListener() {
		...
		public void onLocationChanged(final Location location) {
			...
			if (location.getProvider().equals(LocationManager.GPS_PROVIDER) || mLastLocation == null
					|| mLastLocation.getProvider().equals(LocationManager.NETWORK_PROVIDER)) {
